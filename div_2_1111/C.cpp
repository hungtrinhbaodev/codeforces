...
int main() {
    ...
    while(t--) {
        ...
        int n = _read<int>();
        std::vector<int> a = _reads<int>(n), b = _reads<int>(n);
        int count_zero = 0, count_one = 0;
        bool has_default_zero = false, has_default_one = false;
        for (int i = 0; i < n; i++) {
            if (a[i] != b[i]) {
                if (a[i] == 0) count_zero++;
                else count_one++;
                ...
                if (a[i] == 0) has_default_zero = true;
                else has_default_one = true;
            }
        }
        ...
        if (count_one <=   0) {
            if (count_zero <= 0) {
                ...
                if (has_default_one && has_default_zero) {
                    _println<int>(2);
                }
                else {
                    _println<int>(-1);
...
/**
 * link contest: https://codeforces.com/contest/2247/problem/C
 */