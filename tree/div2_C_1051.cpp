...
struct Node {
    ...
    // node child a contain chunk [e, a, d] (all it's child, grand child,... and it)
...
void dfs_process(std::vector<std::vector<Adjust>>& adjusts, std::vector<Node>& nodes, int cur, int parent) {
...
void dfs_result(std::vector<std::vector<Adjust>>& adjusts, std::vector<Node>& nodes, std::vector<int>& ans, int cur,
                int parent) {
    if (is_leaf_node(adjusts, cur, parent)) {
...
int main() {
    ...
    while (t--) {
        ...
        dfs_process(adjusts, nodes, 0, -1);

        std::vector<int> result(n);
        dfs_result(adjusts, nodes, result, 0, -1);
        ...
        _prints(ans, [](const int& permu) { std::cout << permu << " "; });
    }
...
/**
 * link context: https://codeforces.com/problemset/problem/2143/C
 */