...
void loop_child_node(std::vector<std::vector<int>>& adjusts, int cur, int parent, std::function<void(int, int)> func) {
...
int get_num_child_node(std::vector<std::vector<int>>& adjusts, int cur, int parent) {
    if (is_leaf(adjusts, cur, parent)) {
        return 0;
    }

    int num_child = 0;
    loop_child_node(adjusts, cur, parent, [&num_child](int cur, int parent) { num_child++; });

    return num_child;
}

void dfs(std::vector<std::vector<int>>& adjusts, std::vector<ll>& costs, std::vector<Node>& nodes, int cur,
         int parent) {
    if (is_leaf(adjusts, cur, parent)) {
        nodes[cur].deep = 1;
    ...
    loop_child_node(adjusts, cur, parent,
                    [&adjusts, &costs, &nodes](int cur, int parent) { dfs(adjusts, costs, nodes, cur, parent); });

    ll sum_cost = 0;
    loop_child_node(adjusts, cur, parent,
                    [&sum_cost, &nodes](int cur, int parent) { sum_cost += nodes[cur].sum_cost; });
    nodes[cur].sum_cost = sum_cost + costs[cur];

    ll subtree_value = 0;
    loop_child_node(adjusts, cur, parent, [&subtree_value, &nodes](int cur, int parent) {
    ...
    int deep = 0;
    loop_child_node(adjusts, cur, parent,
                    [&deep, &nodes](int cur, int parent) { deep = std::max(deep, nodes[cur].deep); });
    ...
    if (num_child > 1) {
        ...
        loop_child_node(adjusts, cur, parent,
                        [&qp_deep, &nodes](int cur, int parent) { qp_deep.push(nodes[cur].deep); });
    ...
    loop_child_node(adjusts, cur, parent, [&subtree_ans_value, &nodes](int cur, int parent) {
...
int main() {
    ...
    while (t--) {
        ...
        dfs(adjusts, costs, nodes, 0, -1);

        _prints<Node>(nodes, [](const Node& node) { std::cout << node.subtree_ans_value << " "; });
    }
...
/**
 * link context: https://codeforces.com/contest/2192/problem/D
 */